function [Volume,param]=polygon2voxel(FV,VolumeSize,mode,Yxz,hollow)
...
switch(lower(mode(1:2)))
    case {'au'} % auto
        ...
        param.pt = FV.vertices;
        param.min = min(FV.vertices,[],1);
...
FacesA=double(FV.faces(:,1)); %array N * 1
FacesB=double(FV.faces(:,2)); %array N * 1
FacesC=double(FV.faces(:,3)); %array N * 1
VerticesX=double(FV.vertices(:,1)); %array M*1
VerticesY=double(FV.vertices(:,2)); %array M*1
VerticesZ=double(FV.vertices(:,3)); %array M*1

% Volume size to double
VolumeSize=double(VolumeSize); % 3*1
% Warp = 0; % parameter
